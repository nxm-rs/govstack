...
contract GasTest is Script {
    ...
    address public constant RECIPIENT_1 =
        0x70997970C51812dc3A010C7d01b50e0d17dc79C8;
    address public constant RECIPIENT_2 =
        0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;
    address public constant RECIPIENT_3 =
    ...
    function runBasicScenario() public {
        ...
        AbstractDeployer.TokenConfig memory tokenConfig = AbstractDeployer
            .TokenConfig({
                name: TOKEN_NAME,
                symbol: TOKEN_SYMBOL,
                initialSupply: 0
        ...
        AbstractDeployer.GovernorConfig memory governorConfig = AbstractDeployer
            .GovernorConfig({
                name: GOVERNOR_NAME,
    ...
    function runWithSplitter() public {
        ...
        bytes memory packedData = abi.encodePacked(
            uint16(6000), RECIPIENT_2,  // 60%
            uint16(4000), RECIPIENT_3   // 40%
        );

        AbstractDeployer.SplitterConfig memory splitterConfig = AbstractDeployer
     ...
     */
    function _extractDeploymentAddresses(
        Vm.Log[] memory logs
    )
        internal
        pure
        ...
        )
    {
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                ...
                (, uint256 distributed, ) = abi.decode(
                    logs[i].data,
     ...
     */
    function _verifyBasicDeployment(
        address tokenAddress,
        address governorAddress,
        uint256 totalDistributed
    ) internal view {
        ...
        require(
            keccak256(bytes(governor.name())) ==
                keccak256(bytes(GOVERNOR_NAME)),
            "Governor name mismatch"
        );
        require(
            governor.votingDelay() == VOTING_DELAY,
            "Voting delay mismatch"
        );
        require(
            governor.votingPeriod() == VOTING_PERIOD,
            "Voting period mismatch"
        );

        // Verify distributions
        require(
            token.balanceOf(RECIPIENT_1) == RECIPIENT_1_AMOUNT,
            "Recipient 1 balance mismatch"
        );
        require(
            token.balanceOf(RECIPIENT_2) == RECIPIENT_2_AMOUNT,
            "Recipient 2 balance mismatch"
        );
        require(
            token.balanceOf(RECIPIENT_3) == RECIPIENT_3_AMOUNT,
            "Recipient 3 balance mismatch"
        );
        require(
            totalDistributed ==
                RECIPIENT_1_AMOUNT + RECIPIENT_2_AMOUNT + RECIPIENT_3_AMOUNT,
    ...
    function _verifySplitterDeployment(
        ...
        require(
            totalDistributed == RECIPIENT_1_AMOUNT + RECIPIENT_2_AMOUNT,
            "Total distributed mismatch"
        );
    ...
    function runLargeScaleTest() public {
        ...
        AbstractDeployer.TokenDistribution[]
            memory distributions = new AbstractDeployer.TokenDistribution[](10);
