...
contract DeployerTest is TestHelper {
    function testBasicDeployment() public {
        ...
        AbstractDeployer.TokenDistribution[]
            memory distributions = createBasicTokenDistribution();
    ...
    function testDeploymentWithSplitter() public {
        AbstractDeployer.SplitterConfig
            memory splitterConfig = createBasicSplitterConfig();
        AbstractDeployer.TokenDistribution[]
            memory distributions = createBasicTokenDistribution();
    ...
    function testInvalidTokenConfig() public {
        ...
        AbstractDeployer.TokenConfig
            memory invalidTokenConfig = AbstractDeployer.TokenConfig({
                name: "",
                symbol: TOKEN_SYMBOL,
                initialSupply: 0
            });
        ...
        invalidTokenConfig = AbstractDeployer.TokenConfig({
            name: TOKEN_NAME,
            symbol: "",
            initialSupply: 0
        });
    ...
    function testInvalidGovernorConfig() public {
        ...
        AbstractDeployer.GovernorConfig
            memory invalidGovernorConfig = AbstractDeployer.GovernorConfig({
                name: "",
    ...
    function testInvalidDistributions() public {
        ...
        AbstractDeployer.TokenDistribution[]
            memory invalidDistributions = new AbstractDeployer.TokenDistribution[](
                1
            );
        invalidDistributions[0] = AbstractDeployer.TokenDistribution({
            recipient: address(0),
            amount: 1000
        });
        ...
        AbstractDeployer.TokenDistribution[]
            memory duplicateDistributions = new AbstractDeployer.TokenDistribution[](
                2
            );
        duplicateDistributions[0] = AbstractDeployer.TokenDistribution({
            recipient: USER1,
            amount: 1000
        });
        duplicateDistributions[1] = AbstractDeployer.TokenDistribution({
            recipient: USER1,
            amount: 2000
        });
    ...
    function testInvalidSplitterConfig() public {
        ...
        bytes memory invalidPackedData = abi.encodePacked(
            uint16(6000), PAYEE1,
            uint16(3000), PAYEE2
        );

        AbstractDeployer.SplitterConfig
            memory invalidSplitterConfig = AbstractDeployer.SplitterConfig({
                packedPayeesData: invalidPackedData
            });
        ...
        new TestableDeployer(
            createBasicTokenConfig(),
            createBasicGovernorConfig(),
            invalidSplitterConfig,
            distributions,
            OWNER
        );

        // Invalid payee (zero address)
        bytes memory zeroAddressPackedData = abi.encodePacked(
            uint16(6000), address(0),
            uint16(4000), PAYEE2
        );

        invalidSplitterConfig = AbstractDeployer.SplitterConfig({
            packedPayeesData: zeroAddressPackedData
        });
    ...
    function testValidateSplitterConfig() public pure {
        ...
        AbstractDeployer.SplitterConfig
            memory validSplitterConfig = createBasicSplitterConfig();
        ...
        AbstractDeployer.SplitterConfig memory invalidConfig = AbstractDeployer.SplitterConfig({
            packedPayeesData: invalidData
        });
    ...
    function testCalculateTotalDistribution() public pure {
        ...
        AbstractDeployer.TokenDistribution[]
            memory emptyDistributions = new AbstractDeployer.TokenDistribution[](
                0
            );
        // Test empty array
